...
const tipoChave = chave => Object.keys(CHAVES).find(tipo=>CHAVES[tipo]===chave);
function lerRegistros(chave) { return window.Conta.ler(tipoChave(chave)); }
async function salvarRegistros(chave,valor) {
  if(gravando.has(chave)){mostrarAviso('Aguarde a gravação em andamento.','error');return false;}
  gravando.add(chave);
  try {return await window.Conta.salvar(tipoChave(chave),valor);}
  catch(e){mostrarAviso(window.Conta.mensagemErro(e),'error');return false;}
  finally{gravando.delete(chave);}
}
function exigirAcesso(id){
  if(PuericulturaPermissoes.podeAcessar(id,window.Conta.usuario?.perfil))return true;
  mostrarAviso(window.Conta.usuario?'Esta ferramenta não está disponível para seu perfil.':'Faça login para acessar esta ferramenta.','error');return false;
}
function navegar(id) {
  if(!PuericulturaPermissoes.podeAcessar(id,window.Conta.usuario?.perfil)){
    exigirAcesso(id); id=window.Conta.usuario?'view-dashboard':'view-login';
  }
  if(id==='view-login'&&window.Conta.usuario)id='view-conta';
...
$('btnAcessoLogin').addEventListener('click',event=>{event.preventDefault();navegar(window.Conta.usuario?'view-conta':'view-login');});
function atualizarAcessos(){
  const perfil=window.Conta.usuario?.perfil;
  document.querySelectorAll('[data-target]').forEach(el=>{
    const permitido=PuericulturaPermissoes.podeAcessar(el.dataset.target,perfil);
    el.hidden=!permitido;
    if(el.closest('li')&&el.classList.contains('nav-link'))el.closest('li').hidden=!permitido;
  });
  $('novaCrianca').hidden=perfil!=='enfermeiro';
  $('listaCriancasCadastradas').hidden=perfil!=='enfermeiro';
}
window.addEventListener('conta-alterada',()=>{
  atualizarAcessos();
  for(const id of ['formNovaConsulta','formCrianca','formEducador','formObservacao'])$(id).reset();
...
$('formNovaConsulta').addEventListener('submit',async event=>{
  if(!exigirAcesso('view-nova-consulta')){event.preventDefault();return;}
  ...
  try{const crianca=lerRegistros(CHAVES.criancas).find(c=>c.id===fd.get('crianca'));if(!crianca){mostrarAviso('Cadastre ou selecione uma criança na Caderneta Digital.','error');return;}
    ...
    if(await salvarRegistros(CHAVES.consultas,registros)){mostrarAviso('Consulta completa salva na sua conta.');limparConsulta();navegar('view-caderneta');}
  }catch{}
});
...
$('formCrianca').addEventListener('submit',async event=>{
  if(!exigirAcesso('view-nova-consulta')){event.preventDefault();return;}
  ...
  try{const registros=lerRegistros(CHAVES.criancas);const item={id:fd.get('criancaId')||crypto.randomUUID(),nome:fd.get('criancaNome').trim(),nascimento:fd.get('criancaNascimento'),responsavel:fd.get('criancaResponsavel').trim(),profissional:fd.get('criancaProfissional').trim()};
    ...
    const i=registros.findIndex(r=>r.id===item.id);if(i<0)registros.push(item);else registros[i]=item;
    if(await salvarRegistros(CHAVES.criancas,registros)){$('cadastroCrianca').hidden=true;$('formCrianca').reset();atualizarCadernetaDigital();mostrarAviso('Registro da criança salvo.');}
  }catch{}
});
...
document.querySelector('#view-caderneta').addEventListener('click',async event=>{
  ...
  try{const registros=lerRegistros(CHAVES[b.dataset.tipo]);const i=Number(b.dataset.index),item=registros[i];if(!item)return;
    ...
    if(b.dataset.action==='excluir'){
      ...
      registros.splice(i,1);if(await salvarRegistros(CHAVES[b.dataset.tipo],registros))atualizarCadernetaDigital();
    }
    if(b.dataset.action==='editar'){
      ...
      if(b.dataset.tipo==='consultas'){
        if(!item.id){item.id=crypto.randomUUID();if(!await salvarRegistros(CHAVES.consultas,registros))return;}
...
$('edData').value=dataHoje();$('dataConsulta').value=dataHoje();atualizarAcessos();atualizarSelecaoCriancas();
