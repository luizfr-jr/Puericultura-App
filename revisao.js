// Montagem dos módulos adicionais antes de registrar a navegação.
(() => {
  const dados = window.PUERICULTURA_CONTEUDO;
  const esc = text => String(text).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const lista = items => `<ul class="module-list check">${items.map(t=>`<li>${esc(t)}</li>`).join('')}</ul>`;
  const card = (title, content) => `<section class="module-card"><h3>${esc(title)}</h3>${content}</section>`;
  const field = (name, label, type='text', attrs='') => `<div class="form-group"><label for="${name}">${label}</label><input class="form-control" id="${name}" name="${name}" type="${type}" ${attrs}></div>`;
  const area = (name,label,attrs='') => `<div class="form-group"><label for="${name}">${label}</label><textarea class="form-control" id="${name}" name="${name}" ${attrs}></textarea></div>`;
  const options = (name,label,values,attrs='') => `<div class="form-group"><label for="${name}">${label}</label><select class="form-control" name="${name}" id="${name}" ${attrs}><option value="">Selecione…</option>${values.map(v=>`<option>${esc(v)}</option>`).join('')}</select></div>`;
  const checks = (name,items) => `<div class="check-grid">${items.map((t,i)=>`<label class="check-item"><input type="checkbox" name="${name}" value="${esc(t)}"><span>${esc(t)}</span></label>`).join('')}</div>`;
  const back = '<button class="btn-voltar open-view" data-target="view-dashboard">← Voltar ao Início</button>';
  const view = (id,title,subtitle,content) => `<section id="${id}" class="page-view inner-view">${back}<header class="module-header-text"><h1 tabindex="-1">${title}</h1><p>${subtitle}</p></header>${content}</section>`;
  ...
  const noticias = [['Pós-graduação promove ciclo de formação para enfermeiros e técnicos da rede municipal','pos-graduacao-promove-ciclo-de-formacao-para-enfermeiros-e-tecnicos-da-rede-municipal-de-saude'],['Acadêmicos de Enfermagem desenvolvem ações de vacinação em empresas','academicos-de-enfermagem-desenvolvem-acoes-de-vacinacao-em-empresas'],['Disciplina de Enfermagem promove visitas técnicas a serviços de saúde','disciplina-de-enfermagem-promove-visitas-tecnicas-a-servicos-de-saude'],['Enfermagem promove ação extensionista em casa de passagem','disciplina-da-enfermagem-promove-acao-extensionista-em-casa-de-passagem']];
  main.insertAdjacentHTML('beforeend',view('view-noticias','Notícias e ações em saúde','Seleção de notícias da Universidade Franciscana indicada no projeto.',`<div class="video-grid">${noticias.map(([t,slug])=>`<a class="document-card" href="https://site.ufn.edu.br/pagina/${slug}" target="_blank" rel="noopener noreferrer"><span class="tag">UFN · Enfermagem</span><h2>${t}</h2><span>Ler notícia ↗</span></a>`).join('')}</div>`));

  const cadastro = `<form id="formCrianca"><input type="hidden" id="criancaId" name="criancaId"><div class="form-grid">${field('criancaNome','Nome completo da criança','text','required maxlength="160"')}${field('criancaNascimento','Data de nascimento','date','required')}${field('criancaResponsavel','Responsável pela criança','text','required maxlength="160"')}${field('criancaProfissional','Profissional responsável','text','maxlength="160"')}</div><div class="form-actions"><button type="reset" class="btn btn-cancel">Limpar</button><button type="submit" class="btn btn-primary">Salvar criança</button></div></form>`;
  ...
  document.getElementById('dataConsulta').name='data'; document.getElementById('dataConsulta').removeAttribute('value');
  formConsulta.querySelectorAll('textarea').forEach((el,i)=>{el.name=['queixa','exame','diagnostico','intervencoes','orientacoes'][i];el.id=el.id||el.name;el.maxLength=8000;el.previousElementSibling.setAttribute('for',el.id);});
  const retorno=formConsulta.querySelectorAll('input[type=date]')[1];retorno.name='retorno';retorno.id='retorno';retorno.previousElementSibling.setAttribute('for','retorno');
  document.querySelector('label + #selectCrianca').previousElementSibling.setAttribute('for','selectCrianca');
